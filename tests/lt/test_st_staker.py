...
class StatefulTrader(RuleBasedStateMachine):
    ...
    def _adjust(self, value):
        return value * (10**18 + self.cryptopool.xcp_profit()) // (2 * self.cryptopool.get_virtual_price())
    ...
    def staked_fractions(self):
        ...
        assert abs(lv.admin + lv.total - self._adjust(self.yb_amm.value_oracle()[1] * 10**18 // p_o)) <= 10
        ...
        if hasattr(self, 'lv'):
            if lv.staked_tokens > 0 and self.lv.staked_tokens > 0 and lv.staked_tokens / lv.supply_tokens <= 0.9999:
                assert lv.staked / lv.staked_tokens >= self.lv.staked / (self.lv.staked_tokens + 1)
