...
def test_deposit_withdraw_crvusd_from_wallet(
    ...
    with boa.env.prank(account):
        ...
        assert abs(crvusd_used - crvusd_needed) < 1000, f"Should use approx {crvusd_needed}, but used {crvusd_used}"
        ...
        assert abs(crvusd_returned - crvusd_needed) <= 1000, f"Should return ~{crvusd_needed}, but returned {crvusd_returned}"
