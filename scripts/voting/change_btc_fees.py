...
FEE_A = int(0.05 * 10**18)   # 5%
FEE_B = int(0.03 * 10**18)   # 3%
FEE_C = int(0.02 * 10**18)   # 2%
...
if __name__ == '__main__':
    ...
    twocrypto_deployer = boa.load_partial('contracts/twocrypto_pool/contracts/main/Twocrypto.vy')
    ...
    for m in market_data:
        ...
        actions_a.append(Action(to=factory_owner.address, value=0,
                                data=factory_owner.lt_set_amm_rate.prepare_calldata(m['lt'].address, FEE_A)))
    ...
    for m in market_data:
        actions_b.append(Action(to=comparator.address, value=0,
                                data=comparator.check_nonequal.prepare_calldata(m['cryptopool'].address,
                                                                                ...
                                                                                m['current_price_scale'])))
        actions_b.append(Action(to=comparator.address, value=0,
                                data=comparator.check_equal.prepare_calldata(m['amm'].address, FEE_SELECTOR, FEE_A)))
        actions_b.append(Action(to=factory_owner.address, value=0,
                                data=factory_owner.lt_set_amm_rate.prepare_calldata(m['lt'].address, FEE_B)))
    ...
    for m in market_data:
        actions_c.append(Action(to=comparator.address, value=0,
                                data=comparator.check_nonequal.prepare_calldata(m['cryptopool'].address,
                                                                                ...
                                                                                m['current_price_scale'])))
        actions_c.append(Action(to=comparator.address, value=0,
                                data=comparator.check_equal.prepare_calldata(m['amm'].address, FEE_SELECTOR, FEE_B)))
        actions_c.append(Action(to=factory_owner.address, value=0,
                                data=factory_owner.lt_set_amm_rate.prepare_calldata(m['lt'].address, FEE_C)))
    ...
    for m in market_data:
        actions_d.append(Action(to=comparator.address, value=0,
                                data=comparator.check_nonequal.prepare_calldata(m['cryptopool'].address,
                                                                                ...
                                                                                m['current_price_scale'])))
        actions_d.append(Action(to=comparator.address, value=0,
                                data=comparator.check_equal.prepare_calldata(m['amm'].address, FEE_SELECTOR, FEE_C)))
        actions_d.append(Action(to=factory_owner.address, value=0,
                                data=factory_owner.lt_set_amm_rate.prepare_calldata(m['lt'].address, m['current_fee'])))

    all_votes = [
        ('A', actions_a, 'Raise BTC market fees to 5% (price_scale unchanged)',
         'Set AMM fee to 5% for WBTC, cbBTC, tBTC markets. '
         'Guarded: only executes if price_scale has not changed and within 3 weeks.'),
        ('B', actions_b, 'Lower BTC market fees from 5% to 3% (price_scale changed)',
         'Set AMM fee to 3% for BTC markets after price_scale change. '
         'Guarded: only executes if price_scale changed, fee is 5%, and within 3.5 weeks.'),
        ('C', actions_c, 'Lower BTC market fees from 3% to 2% (price_scale changed)',
         'Set AMM fee to 2% for BTC markets. '
         'Guarded: only executes if price_scale changed, fee is 3%, and within 3.5 weeks.'),
        ('D', actions_d, 'Lower BTC market fees from 2% to original (price_scale changed)',
         'Restore original AMM fee for BTC markets. '
    ...
    if FORK:
        print("\n=== Simulating Vote A (fee -> 5%, price_scale unchanged) ===")
        ...
        print("\n=== Vote D should revert (fee is 5%, not 2%) ===")
        try:
            with boa.env.prank(DAO):
                for action in actions_d:
                    boa.env.raw_call(to_address=action.to, data=action.data)
            print("  ERROR: Vote D should have reverted!")
        except Exception as e:
            print(f"  Correctly reverted: {e}")
